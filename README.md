...
## Basic usage
Populate `src/config/config.yaml` and run the app:
```
...
## Configuration file
`src/config/config.yaml` controls non-interactive runs:
```yaml
spreadsheet_id: "your-sheet-id"
config_xlsx: "src/cfg/Schedule.xlsx"   # optional; defaults to src/cfg/Schedule.xlsx
...
1. Every worksheet in `src/cfg/Schedule.xlsx` is scanned and every cell whose trimmed text equals `โอเลี้ยง` is collected.
...
## Interactive choices
If `src/config/config.yaml` is absent you will be prompted to:
- Enter the spreadsheet ID.
- Provide the Excel workbook path (defaults to `src/cfg/Schedule.xlsx`) and optional sheet filter, plus the lookup value to locate inside the workbook.
...
- Delete or adjust `src/config/config.yaml` when switching to a different spreadsheet.
